...
if COMPRESS_IMAGES:
	def skill(skill_name: str) -> str:
		imgmck=subprocess.run(['magick', f'skill_icons/{skill_name}.svg', '-geometry', 'x24', 'JPG:-'], stdout=subprocess.PIPE)

		if imgmck.returncode==0:
			return f'<button class="skill_badge"><img class="skill_badge_img" src="data: image/jpeg; base64, {base64.b64encode(imgmck.stdout).decode()}"></button>'
